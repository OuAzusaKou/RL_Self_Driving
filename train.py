...
def evaluate(env, agent, video, num_episodes, L, step, args,model_dir):
    global best_ep_reward
    ...
    def run_eval_loop(sample_stochastically=True):
        ...
        new_best_ep_reward = np.max(all_ep_rewards)

        if new_best_ep_reward > best_ep_reward:
            print('we get the new_best_ep_reward:',new_best_ep_reward)
...
def main():
    ...
    for step in range(args.num_train_steps):
        ...
        if step > 0:
            if step % args.eval_freq == 0:
                evaluate(env, agent, video, args.num_eval_episodes, L, step, args,model_dir)
